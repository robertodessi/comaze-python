...
def pair_two_agents_and_play_one_iterated_game(task) -> List[Union[bool, str]]:
    ...
    # across all games for one pair, we have the same instances
    #agent1, agent2 = agent1(), agent2()
    agent1.player = agent1.player()
    agent2.player = agent2.player()
...
def pair_all_agents_and_play_all_games(players: List[Agent]):
    ...
    for idx_agent1 in range(len(players)-1):
        for idx_agent2 in range(idx_agent1+1, len(players)):
            ...
            tuple_tasks.append((agent1, agent2, games_rates[counter]))
            counter += 1
    ...
    if DEBUG:
        games_result = [pair_two_agents_and_play_one_iterated_game(tuple_task) for tuple_task in tuple_tasks]
    else:
        ...
        with Pool(n_processes) as p:
            ...
            for promise in promised_games_result:
                try:
                    ...
                    games_result.append([(False, "<unknown-game-id:timeout>")] * 4)
                except:
                    games_result.append([(False, "<unknown-game-id:died>")] * 4)
            p.close()
            # p.join() # never returns if a worker is busy forever

    for task, game_result in zip(tuple_tasks, games_result):
        agent1, agent2, _ = task
        for level, game in enumerate(game_result, start=1):
            game_won, game_id = game
